...
describe('Clearing House Scenario 7 (Liquidation | Account Positive | Account Value > .75 * MM & PositionNotional < MinNotionalLiquidatable )', () => {
  ...
  describe('#Collateral Deposit', async () => {
    ...
    it('Acct[1] Initial Collateral Deposit = 50 USDC', async () => {
      await addMargin(user1, user1AccountNo, settlementToken.address, parseTokenAmount(5n * 10n ** 1n, 6));
      await checkSettlementVTokenBalance(user1.address, parseTokenAmount(5n * 10n ** 6n - 5n * 10n ** 1n, 6));
      await checkSettlementVTokenBalance(
        clearingHouseTest.address,
        parseTokenAmount(5n * 10n ** 6n + 5n * 10n ** 1n, 6),
      );
      await checkDepositBalance(user1AccountNo, settlementToken.address, parseTokenAmount(5n * 10n ** 1n, 6));
    });
    ...
    it('Acct[2] Initial Collateral Deposit = 10m USDC', async () => {
      ...
      await checkSettlementVTokenBalance(
        clearingHouseTest.address,
        parseTokenAmount(15n * 10n ** 6n + 5n * 10n ** 1n, 6),
      );
    ...
    it('Keeper Initial Collateral Deposit = 1m USDC', async () => {
      ...
      await checkSettlementVTokenBalance(
        clearingHouseTest.address,
        parseTokenAmount(16n * 10n ** 6n + 5n * 10n ** 1n, 6),
      );
  ...
  describe('#Scenario Partial Token Liquidation', async () => {
    ...
    it('Timestamp And Oracle Update - 0', async () => {
      ...
      const realSqrtPrice = await priceToSqrtPriceX96(2511.18799498702, vQuote, vToken);
      await oracle.setSqrtPriceX96(realSqrtPrice);
    });

    it('Acct[0] Adds Liq to ETH Pool b/w ticks (-198500 to -191200) @ tickCurrent = -198045', async () => {
      const tickLower = -198500;
      const tickUpper = -191200;
      const liquidityDelta = 500000000000000n;
      const limitOrderType = 0;
      const expectedToken2Balance = -2893154962442030000n;
      const expectedVQuoteBalance = -563276561n - 1n;
    ...
    it('Acct[1] Short ETH : Price Changes (StartTick = -198045, EndTick = -198080)', async () => {
      const startTick = -198045;
      const endTick = -198080;

      const swapTokenAmount = '-17484125147797800';
      const expectedVTokenBalance = '-17484125147797800';

      //TODO: Check
      const expectedVQuoteBalance = 43719676n;

      const expectedTokenAmountOut = swapTokenAmount;
      const expectedVQuoteAmountOutWithFee = 43719676n;
    ...
    it('Acct[2] Long ETH : Price Changes (StartTick = -198080, EndTick = -191380)', async () => {
      const startTick = -198080;
      const endTick = -191380 - 1;

      const swapToken2Amount = '2846548513603800000';
      const expectedToken2Balance = '2846548513603800000';
      const expectedVQuoteBalance = -9962674012n - 5n;

      // const expectedSumB = ((2494598646n*(1n<<128n))/(10n**13n))+1n;

      const expectedTokenAmountOut = swapToken2Amount;
      const expectedVQuoteAmountOutWithFee = -9962674012n - 5n;
      const expectedFundingPayment = 0n;
    ...
    it('Timestamp and Oracle Update - 2000', async () => {
      ...
      const realSqrtPrice = await priceToSqrtPriceX96(4887.70392470383, vQuote, vToken);
      await oracle.setSqrtPriceX96(realSqrtPrice);
    });

    it('Acct[1] Underwater : Liquidate ETH Token Positions @ current tickETH = -191380', async () => {
      ...
      const expectedInsuranceFundFee = 1286283n;

      const liquidatorsettlementVTokenBalance = 1286283n;
