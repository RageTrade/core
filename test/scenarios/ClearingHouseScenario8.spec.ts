...
describe('Clearing House Scenario 8 (Liquidation | Account Position | Partial Liquidation (Account Value > .75 * MM & PositionNotional > MinNotionalLiquidatable)', () => {
  ...
  describe('#Scenario Partial Token Liquidation', async () => {
    ...
    it('Acct[2] Long ETH : Price Changes (StartTick = -198080, EndTick = -196850)', async () => {
      const startTick = -198080;
      const endTick = -196750 - 1;

      const swapToken2Amount = '514680679981391000000';
      const expectedToken2Balance = '514680679981391000000';
      const expectedVQuoteBalance = -1377187137427n + 8n;

      // const expectedSumB = ((2494598646n*(1n<<128n))/(10n**13n))+1n;

      const expectedTokenAmountOut = swapToken2Amount;
      const expectedVQuoteAmountOutWithFee = -1377187137427n + 8n;
      const expectedFundingPayment = 0n;
    ...
    it('Timestamp and Oracle Update - 2000', async () => {
      ...
      const realSqrtPrice = await priceToSqrtPriceX96(2856.93535575608, vQuote, vToken);
      await oracle.setSqrtPriceX96(realSqrtPrice);
    });

    it('Acct[1] Underwater : Liquidate ETH Token Positions @ current tickETH = -196750', async () => {
      // const {virtualPriceX128} = await clearingHouseTest.getTwapPrices(vToken.address);
      // console.log("Twap: ", virtualPriceX128.mul(10n**30n).div(1n<<128n))

      const expectedVTokenBalance = -49687187855000000000n;
      ...
      const expectedInsuranceFundFee = 2145666844n;

      const liquidatorSettlementVTokenBalance = 2145666844n;
