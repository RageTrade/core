...
describe('Clearing House Scenario 9 (Settle Profit)', () => {
  ...
  describe('#Collateral Deposit', async () => {
    ...
    it('Acct[0] Initial Collateral Deposit = 1M USDC', async () => {
      await addMargin(user0, user0AccountNo, settlementToken.address, parseTokenAmount(1n * 10n ** 6n, 6));
      await checkSettlementVTokenBalance(user0.address, parseTokenAmount(0n, 6));
      await checkSettlementVTokenBalance(clearingHouseTest.address, parseTokenAmount(1n * 10n ** 6n, 6));
      await checkDepositBalance(user0AccountNo, settlementToken.address, parseTokenAmount(1n * 10n ** 6n, 6));
    });

    it('Acct[1] Initial Collateral Deposit = 1M USDC', async () => {
      await addMargin(user1, user1AccountNo, settlementToken.address, parseTokenAmount(1n * 10n ** 6n, 6));
      ...
      await checkDepositBalance(user1AccountNo, settlementToken.address, parseTokenAmount(1n * 10n ** 6n, 6));
    });

    it('Acct[2] Initial Collateral Deposit = 1M USDC', async () => {
      await addMargin(user2, user2AccountNo, settlementToken.address, parseTokenAmount(1n * 10n ** 6n, 6));
      await checkSettlementVTokenBalance(user2.address, parseTokenAmount(0n, 6));
      await checkSettlementVTokenBalance(clearingHouseTest.address, parseTokenAmount(3n * 10n ** 6n, 6));
      await checkDepositBalance(user2AccountNo, settlementToken.address, parseTokenAmount(1n * 10n ** 6n, 6));
    });

    it('Keeper Initial Collateral Deposit = 1M USDC', async () => {
      await addMargin(keeper, keeperAccountNo, settlementToken.address, parseTokenAmount(1n * 10n ** 6n, 6));
      await checkSettlementVTokenBalance(keeper.address, parseTokenAmount(0n, 6));
      await checkSettlementVTokenBalance(clearingHouseTest.address, parseTokenAmount(4n * 10n ** 6n, 6));
      await checkDepositBalance(keeperAccountNo, settlementToken.address, parseTokenAmount(1n * 10n ** 6n, 6));
    });
  });
  ...
  describe('#Scenario Liquidation', async () => {
    ...
    it('Timestamp And Oracle Update - 0', async () => {
      ...
      const realSqrtPrice1 = await priceToSqrtPriceX96(61362.1958892546, vQuote, vToken1);
    ...
    it('Acct[1] Long BTC : Price Changes (StartTick = 64197, EndTick = 65000)', async () => {
    ...
    it('Timestamp and Oracle Update - 1000', async () => {
      ...
      const realSqrtPrice1 = await priceToSqrtPriceX96(66492.5511541868, vQuote, vToken1);
      await oracle1.setSqrtPriceX96(realSqrtPrice1);
      const realSqrtPrice = await priceToSqrtPriceX96(3626.38967029497, vQuote, vToken);
      await oracle.setSqrtPriceX96(realSqrtPrice);
    });

    it('Acct[2] Long BTC : Price Changes (StartTick = 65000, EndTick = 66000)', async () => {
      const startTick = 65000;
      const endTick = 66000;
    ...
    it('Timestamp and Oracle Update - 2000', async () => {
      ...
      const realSqrtPrice1 = await priceToSqrtPriceX96(73485.266401505, vQuote, vToken1);
      await oracle1.setSqrtPriceX96(realSqrtPrice1);
    });

    it('Acct[1] Partial Close Position, Short BTC : Price Changes (StartTick = 66000, EndTick = 65500)', async () => {
      const startTick = 66000;
      const endTick = 65500;
    ...
    it('Timestamp And Oracle Update - 3000', async () => {
      ...
      const realSqrtPrice1 = await priceToSqrtPriceX96(69901.5224103246, vQuote, vToken1);
      await oracle1.setSqrtPriceX96(realSqrtPrice1);
    });

    it('Acct[0] Remove Liq from BTC Pool b/w ticks (60000 to 68000) @ tickCurrent = 65500', async () => {
      const tickLower = 60000;
      const tickUpper = 68000;
