...
describe('Clearing House Scenario 3 (Underwater Liquidation)', () => {
  ...
  describe('#Initialize', () => {
    it('Steal Funds', async () => {
      ...
      await stealFunds(REAL_BASE, 6, insuranceFund.address, '1000000', whaleForBase);
  ...
  describe('#Scenario Underwater Liquidation', async () => {
    ...
    it('Acct[2] Long BTC : Price Changes (StartTick = 64400, EndTick = 67000)', async () => {
      ...
      //TODO:Check
      const expectedBaseBalance = -3432640828758n - 1n;

      // const expectedSumB = ((2494598646n*(1n<<128n))/(10n**13n))+1n;

      const expectedTokenAmountOut = swapToken1Amount;

      const expectedBaseAmountOutWithFee = -2609309782963n - 1n;
    ...
    it('Acct[1] Underwater : Liquidate BTC Token Positions @ current tickBTC = 67000', async () => {
      ...
      const expectedBaseBalance = tokenAmount(-100000n, 6);
