...
type VTokenType is address;

library VTokenLib {
    using VTokenLib for VTokenType;
    using FullMath for uint256;

    function iface(VTokenType vToken) internal pure returns (IVToken) {
        return IVToken(VTokenType.unwrap(vToken));
    }

    function realToken(VTokenType vToken) internal view returns (IERC20) {
        return IERC20(vToken.iface().realToken());
    }

    function isToken0(VTokenType vToken) internal pure returns (bool) {
        return VTokenType.unwrap(vToken) < VBASE_ADDRESS;
    }

    function isToken1(VTokenType vToken) internal pure returns (bool) {
    ...
    function vPool(
        ...
        address token0;
        address token1;
        address vTokenAddress = VTokenType.unwrap(vToken);
    ...
    // overload
    function vPool(VTokenType vToken) internal pure returns (IUniswapV3Pool) {
    ...
    function vPoolWrapper(
        ...
        return
            IVPoolWrapper(
                Create2.computeAddress(
                    keccak256(abi.encode(VTokenType.unwrap(vToken), VBASE_ADDRESS)),
    ...
    // overload
    function vPoolWrapper(VTokenType vToken) internal pure returns (IVPoolWrapper) {
        return vToken.vPoolWrapper(WRAPPER_BYTE_CODE_HASH, VPOOL_FACTORY);
    }

    function getVirtualTwapSqrtPrice(VTokenType vToken) internal view returns (uint160 sqrtPriceX96) {
        return Oracle.getTwapSqrtPrice(vToken.vPool(), vToken.vPoolWrapper().timeHorizon());
    }

    function getRealTwapSqrtPrice(VTokenType vToken) internal view returns (uint160 sqrtPriceX96) {
        return IOracleContract(vToken.iface().oracle()).getTwapSqrtPrice(vToken.vPoolWrapper().timeHorizon());
    }

    function getVirtualTwapTick(VTokenType vToken) internal view returns (int24 tick) {
        return Oracle.getTwapTick(vToken.vPool(), vToken.vPoolWrapper().timeHorizon());
    }

    function getVirtualTwapPrice(VTokenType vToken) internal view returns (uint256 price) {
    ...
    function getRealTwapPrice(VTokenType vToken) internal view returns (uint256 price) {
        uint256 sqrtPriceX96 = vToken.getRealTwapSqrtPrice();
        return sqrtPriceX96.mulDiv(sqrtPriceX96, FixedPoint96.Q96); // TODO refactor this
    }

    function getMarginRatio(VTokenType vToken, bool isInitialMargin) internal view returns (uint16) {
        if (isInitialMargin) {
